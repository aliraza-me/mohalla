...
class Soo_Product_Attribute_Swatch_Frontend {
	/**
	 * The single instance of the class
	 *
	 * @var Soo_Product_Attribute_Swatch_Frontend
	 */
	...
	public function __construct() {
		...
		add_filter( 'soopas_swatch_html', array( $this, 'swatch_html' ), 5, 4 );
	}
	...
	public function enqueue_scripts() {
		wp_enqueue_style( 'soopas-frontend', plugins_url( 'assets/css/frontend.css', dirname( __FILE__ ) ), array(), '20160615' );
		wp_enqueue_script( 'soopas-frontend', plugins_url( 'assets/js/frontend.js', dirname( __FILE__ ) ), array( 'jquery' ), '20160615', true );
	}
	...
	public function get_swatch_html( $html, $args ) {
		$swatch_types = Soo_Product_Attribute_Swatches()->types;
		$attr         = Soo_Product_Attribute_Swatches()->get_tax_attribute( $args['attribute'] );
		...
		if ( array_key_exists( $attr->attribute_type, $swatch_types ) ) {
			if ( ! empty( $options ) && $product && taxonomy_exists( $attribute ) ) {
				...
				foreach ( $terms as $term ) {
					if ( in_array( $term->slug, $options ) ) {
						$swatches .= apply_filters( 'soopas_swatch_html', '', $term, $attr, $args );
					}
				}
			}
			...
			if ( ! empty( $swatches ) ) {
				...
				$swatches = '<div class="soopas-swatches" data-attribute_name="attribute_' . esc_attr( $attribute ) . '">' . $swatches . '</div>';
