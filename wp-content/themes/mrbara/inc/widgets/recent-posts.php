...
class MrBara_Recent_Posts_Widget extends WP_Widget {
	/**
	 ...
	 * Constructor
	 *
	 * @return MrBara_Recent_Posts_Widget
	 */
	function __construct() {
		...
		parent::__construct(
			'recent-posts-widget',
			esc_html__( 'MrBara - Recent Posts', 'mrbara' ),
			array(
				'classname'   => 'recent-posts-widget',
				'description' => esc_html__( 'Advanced recent posts widget.', 'mrbara' )
			),
			array( 'width' => 590 )
		);
	}
		...
		while ( $query->have_posts() ) : $query->the_post();
			...
			<article class="recent-post <?php echo esc_attr( $class ); ?>">
				...
				<div class="post-text">
					...
					<?php
					if ( $instance['date'] ) {
						echo '<time class="post-date" datetime="' . esc_attr( get_the_time( 'c' ) ) . '">' . get_the_time( ' F, dS, Y' ) . '</time>';
					}
					?>
				</div>
			</article>
	...
	function form( $instance ) {
		...
		<div class="mr-recent-box">
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'limit' ) ); ?>"><?php esc_html_e( 'Number Of Posts', 'mrbara' ); ?></label>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>"><?php esc_html_e( 'Select Category: ', 'mrbara' ); ?></label>
				<select class="widefat" multiple="multiple" id="<?php echo esc_attr( $this->get_field_id( 'cat' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'cat' ) ); ?>[]">
					<option value="" <?php selected( empty( $instance['cat'] ) ); ?>><?php esc_html_e( 'All Categories', 'mrbara' ); ?></option>
					<?php
		...
		<div class="mr-recent-box last">
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'date' ) ); ?>"><?php esc_html_e( 'Show Date', 'mrbara' ); ?></label>
			</p>
			...
			<p>
				...
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb' ) ); ?>"><?php esc_html_e( 'Show Thumbnail', 'mrbara' ); ?></label>
			</p>

			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'thumb_size' ) ); ?>"><?php esc_html_e( 'Thumbnail Size', 'mrbara' ); ?></label>
